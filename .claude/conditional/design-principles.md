...
- **Prefer emitting equivalent IR over threading conventions through multiple layers.** If a feature can be expressed using existing constructs, do that.
...
- **Do not encode information in string representations.** Use typed objects. Never use string prefixes, patterns, or regex to deduce what a value represents — use type checks.
...
- Dependency injection for external systems (databases, OS, file I/O, clocks, GUIDs).
- No static methods.
